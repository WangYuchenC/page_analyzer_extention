...
- **`execute_script`**: Routes through `sendMessage()` to background.ts, which uses CDP `Runtime.evaluate` to bypass page CSP (content script `new Function()` would be blocked). Debugger attach/detach lifecycle managed with `needsDetach` flag — only detaches if we attached, won't interfere with network monitoring. `finally` block guarantees cleanup. Uses `returnByValue: true` for JSON-serializable results; handles `exceptionDetails`, non-serializable return values, and edge cases.
