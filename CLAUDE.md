...
- **`reasoning_content` streaming**: DeepSeek thinking mode handled in-stream — `reasoning_content` captured from streaming chunks and yielded as labeled content (`[思考过程]...[/思考过程]`), preserved in `additional_kwargs` via `accumulated.concat()`. No non-streaming fallback needed.
